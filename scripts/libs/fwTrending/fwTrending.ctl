...
@par Modification History
  ...
  07/08/2014: Marco Boccioli
  Fix for  FWTREND-934
  - Error when clicking on Log checkbox. Function fwTrendingControl_panel_autoScalePushed() was renamed to fwTrendingControl_autoScalePushed()

 03/07/2014: Marco Boccioli
  Fix for FWTREND-924
  ...
  16/06/2014: Marco Boccioli
	Fix for FWTREND-916,FWTREND-925
	-Merge ATLAS specific customizations
	-Duplicate the commands contained in of the Control bar Menu to the right-click menu
	-Added all menu entries to the right-click menu.
	-All captions backgrounds are now inheriting background from trending area.

  03/06/2014: Marco Boccioli
	Fix for FWTREND-917
  - check if shape trendCaption.currentTime.currentTime exists

  29/05/2014: Marco Boccioli
	Fix for FWTREND-915
  - moved and renamed function fwTrending_getAndSaveFromTrendWidget() from fwTrending/fwTrendingTrendControl.pnl to fwTrending.ctl
  - moved and renamed constant fwTrending_REFRESH_DELAY from fwTrending/fwTrendingTrendControl.pnl to fwTrending.ctl

  28/08/2012: Marco Boccioli
	Fix for FWTREND-912
	- Dependency on Unicos variable UN_DEVICE_DYNAMICTREND hidden under check if unCore is installed.

  28/08/2012: Marco Boccioli
	Fix for FWTREND-883
	- fwTrending_exportTrend() uses bonus value if the option fillEmptyCells=true
...
const string fwTrending_PLOT_SAVE_TO_USER_FOLDER  = ".saveToUserFolder";


// Kept for compatibility - should always be equal to fwTrending_PLOT_CURVE_TYPES
const string fwTrending_CURVE_TYPES = fwTrending_PLOT_CURVE_TYPES;
...
const int fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER        = 46;
...
const int fwTrending_SIZE_PLOT_OBJECT  = 46;
...
const int fwTrending_getPlotData_LEN_RETURN_DATA = fwTrending_SIZE_PLOT_OBJECT; // 46;
...
const int fwTrending_REFRESH_DELAY = 100;
...
void fwTrending_newPage(string _pageName, string model = fwTrending_YT_PAGE_MODEL)
{
    ...
    string sMaxCols, sMaxRows;
	dyn_string exceptionInfo;
	dyn_dyn_string data;

    sMaxCols = "TrendingConfiguration.PageSettings.maxColumns:_original.._value";
  ...
  if ( (sMaxCols != "") && (dpExists(sMaxCols)) )
  {
  	dpGet(sMaxCols, maxCols);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> fwTrending_newPage() -> sMaxCols DPE is empty or doesn't exist: " + sMaxCols);
    return;
  }
  ...
  {
  	dpGet(sMaxRows, maxRows);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> fwTrending_newPage() -> sMaxRows DPE is empty or doesn't exist: " + sMaxRows);
    return;
  }
...
@param sPlotName    Input, string, the data point name of the plot to create
@param sModel       Input, string  OPTIONAL PARAMETER - default value fwTrending_YT_PLOT_MODEL (value over time plot)
                                     Defines the type of plot to be created, e.g.
                                       fwTrending_YT_PLOT_MODEL
                                       fwTrending_XY_PLOT_MODEL
                                       fwTrending_HISTOGRAM_PLOT_MODEL
...
void fwTrending_newPlot(string sPlotName, string sModel = fwTrending_YT_PLOT_MODEL)
{
  ...
  dpCreate(sPlotName, fwTrending_PLOT);

  //if the default settings exist, use them
  if(dpExists(fwTrending_PLOT_DEFAULTS) && fwTrending_PLOT_DEFAULTS != sPlotName)
  {
    fwTrending_getPlot(fwTrending_PLOT_DEFAULTS , ddsData, exceptionInfo);

    // Remove plot-specific information
    ddsData[fwTrending_PLOT_OBJECT_DPES]    = makeDynString("", "", "", "", "", "", "", "");
    ddsData[fwTrending_PLOT_OBJECT_DPES_X]  = makeDynString("", "", "", "", "", "", "", "");
    ddsData[fwTrending_PLOT_OBJECT_TITLE]   = makeDynString(sPlotName);
    ddsData[fwTrending_PLOT_OBJECT_LEGENDS] = makeDynString("", "", "", "", "", "", "", "");
  }
  else
  {
    ddsData[fwTrending_PLOT_OBJECT_MODEL]                = sModel;
    ddsData[fwTrending_PLOT_OBJECT_TITLE]                = sPlotName;
    ddsData[fwTrending_PLOT_OBJECT_LEGEND_ON]            = TRUE;
    ddsData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON]       = fwTrending_HIDE_CAPTIONS;
    ddsData[fwTrending_PLOT_OBJECT_BACK_COLOR]           = "FwTrendingTrendBackground";
    ddsData[fwTrending_PLOT_OBJECT_FORE_COLOR]           = "FwTrendingTrendForeground";
    ddsData[fwTrending_PLOT_OBJECT_DPES]                 = makeDynString();
    ddsData[fwTrending_PLOT_OBJECT_LEGENDS]              = makeDynString();
    ddsData[fwTrending_PLOT_OBJECT_COLORS]               = makeDynString("FwTrendingCurve1", "FwTrendingCurve2", "FwTrendingCurve3", "FwTrendingCurve4",
                                                                         "FwTrendingCurve5", "FwTrendingCurve6", "FwTrendingCurve7", "FwTrendingCurve8");
    ddsData[fwTrending_PLOT_OBJECT_LEGENDS]              = makeDynBool();
    ddsData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN]        = makeDynBool(0, 0, 0, 0, 0, 0, 0, 0);
    ddsData[fwTrending_PLOT_OBJECT_RANGES_MIN]           = makeDynFloat();
   	ddsData[fwTrending_PLOT_OBJECT_RANGES_MAX]           = makeDynFloat();
    ddsData[fwTrending_PLOT_OBJECT_TYPE]                 = fwTrending_PLOT_TYPE_LINEAR;
    ddsData[fwTrending_PLOT_OBJECT_TIME_RANGE]           = fwTrending_SECONDS_IN_ONE_HOUR;
    ddsData[fwTrending_PLOT_OBJECT_TEMPLATE_NAME]        = "";
    ddsData[fwTrending_PLOT_OBJECT_IS_TEMPLATE]          = FALSE;
    ddsData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC]       = FALSE;
    ddsData[fwTrending_PLOT_OBJECT_GRID]                 = TRUE;
    ddsData[fwTrending_PLOT_OBJECT_MARKER_TYPE]          = fwTrending_MARKER_TYPE_FILLED_CIRCLE;
    ddsData[fwTrending_PLOT_OBJECT_ACCESS_CONTROL_SAVE]  = "";
    ddsData[fwTrending_PLOT_OBJECT_CURVE_TYPES]          = makeDynInt(fwTrending_PLOT_TYPE_STEPS, fwTrending_PLOT_TYPE_STEPS, fwTrending_PLOT_TYPE_STEPS, fwTrending_PLOT_TYPE_STEPS,
                                                                      fwTrending_PLOT_TYPE_STEPS, fwTrending_PLOT_TYPE_STEPS, fwTrending_PLOT_TYPE_STEPS, fwTrending_PLOT_TYPE_STEPS);
    ddsData[fwTrending_PLOT_OBJECT_AXII]                 = makeDynInt(1, 1, 1, 1, 1, 1, 1, 1);
    ddsData[fwTrending_PLOT_OBJECT_AXII_POS]             = makeDynInt(2, 2, 2, 2, 2, 2, 2, 2); //2 = SCALE_LEFT, 3 = SCALE_RIGHT
    ddsData[fwTrending_PLOT_OBJECT_AXII_LINK]            = makeDynInt(1, 1, 1, 1, 1, 1, 1, 1);
    ddsData[fwTrending_PLOT_OBJECT_DEFAULT_FONT]         = fwTrending_DEFAULT_FONT;
    ddsData[fwTrending_PLOT_OBJECT_CURVE_STYLE]          = 0;
    ddsData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT]        = makeDynString("%X", "%x", "", "", "", "", "", "");
    ddsData[fwTrending_PLOT_OBJECT_AXII_Y_FORMAT]        = makeDynString("", "", "", "", "", "", "", "");
    ddsData[fwTrending_PLOT_OBJECT_LEGEND_VALUES_FORMAT] = makeDynString("", "", "", "", "", "", "", "");
    ddsData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW]    = makeDynBool();
    ddsData[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER]  = makeDynBool(FALSE);
  }

  fwTrending_setPlot(sPlotName, ddsData, exceptionInfo);
							 ...
							 dyn_string curveTimeObj, bool bEnabledVisibility)
{
	...
	unsigned updateRate;
	string symbolName = $sRefName + "trend.";
	shape standardTrend = getShape(symbolName + "standardTrend");
	string sBackCol =   standardTrend.backCol;
	string sForeCol =   standardTrend.foreCol;
	...
	if((dsDpe[indexToCheck] != "") && (dsDpe[fwTrending_Y_VALUE] != ""))
	{
		for(i = 1; i <= dynlen(dsDpe); i++)
		{
			...
			if(sColor != "")
			{
				setMultiValue(legendObj[i], "foreCol", sColor,
							  valueObj[i], "foreCol", sColor,
							  unitObj[i], "foreCol", sColor);
			}
			else
			{
				setMultiValue(legendObj[i], "foreCol", "FwTrendingCurve" + curveNumber,
							  valueObj[i], "foreCol", "FwTrendingCurve" + curveNumber,
							  unitObj[i], "foreCol", "FwTrendingCurve" + curveNumber);
			}
			setMultiValue(legendObj[i], "backCol", sBackCol,
						  valueObj[i], "backCol", sBackCol,
						  unitObj[i], "backCol", sBackCol,
						  curveTimeObj[i], "backCol", sBackCol);
			...
			setMultiValue(valueObj[i], "toolTipText", sCurveToolTip,
						  legendObj[i], "toolTipText", sCurveToolTip);

			// set the unit
			setValue(unitObj[i], "text", dsUnit[i]);

			// set the valueCurve, legend, visibility and unit to visible
			setMultiValue(legendObj[i], "visible", true,
						  valueObj[i], "visible", true,
						  unitObj[i], "visible", true);
			setMultiValue("currentTime", "backCol", sBackCol,
						  "currentTime", "foreCol", sForeCol);
	...
	else
	{
		...
		setMultiValue(visibilityObj, "visible", true,
					  visibilityObj, "enabled", bEnabledVisibility,
					  visibilityObj, "visible", false,
					  visibilityObj, "state", 0, false);
	}
	if(shapeExists($sRefName + "trendCaption.captionsBackground"))
	{
		setValue($sRefName + "trendCaption.captionsBackground", "backCol", sBackCol);
	}
	else if(shapeExists(symbolName + "trendCaption.captionsBackground"))
	{
		setValue(symbolName + "trendCaption.captionsBackground", "backCol", sBackCol);
	}
	else
	{
		DebugTN("_fwTrending_initTrendCaption(): can't find object " + symbolName + "trendCaption.captionsBackground");
	}
}
...
{
	bool yIsLog, isLinked, limitsShown, isAuto, alertExists, isActive;
	int alertType;
		...
		if(alertType != DPCONFIG_NONE)
		{
			...
			for(i = okRange; i <= dynlen(alertLimits); i++)
			{
				...
				if(dynContains(existingCurves, curveName + "_limit_" + i) == 0)
				{
                    ...
                    if( (alertClasses[i + 1] != "") && (dpExists(alertClasses[i + 1] + ":_alert_class.._color_c_ack")) )
                    {
                    ...
                    {
                      DebugTN("Error in fwTrending.ctl -> fwTrending_toggleAlarmLimits() -> alertClasses[i + 1]:_alert_class.._color_c_ack DPE is empty or doesn't exist: " + alertClasses[i + 1] + ":_alert_class.._color_c_ack");
                      return;
                    }
			...
			for(i = okRange - 1; i > 0; i--)
			{
				...
				if(dynContains(existingCurves, curveName + "_limit_" + i) == 0)
				{
                    ...
                    if( (alertClasses[i] != "") && (dpExists(alertClasses[i] + ":_alert_class.._color_c_ack")) )
                    {
                    ...
                    {
                      DebugTN("Error in fwTrending.ctl -> fwTrending_toggleAlarmLimits() -> alertClasses[i]:_alert_class.._color_c_ack DPE is empty or doesn't exist: " + alertClasses[i] + ":_alert_class.._color_c_ack");
                      return;
                    }
					activeTrendShape.curveValue(curveName + "_limit_" + i, alertLimits[i],
												makeTime(0, 0, 0, 0), (bit32)15);
							   ...
							   bool passingCurveVisible = FALSE)
{
	...
	if(runTrend)
	{
		if(shapeExists(ref + "trendCaption.currentTime.currentTime"))
		{
			setValue(ref + "trendCaption.currentTime.currentTime", "visible", runTrend);
		}
		else
		{
			setValue(ref + "trendCaption.currentTime", "visible", runTrend);
		}
...
fwTrending_controlBarOnOff(string ref, int visible)
{
	...
	{
		//pick the legend that is the lowest down in the panel (depends on the panel)
		int iLegendY1, iLegendY2;
		getValue(cmdRef + "Text3", "position", iLegendX, iLegendY1);
		getValue(cmdRef + "Text8", "position", iLegendX, iLegendY2);
		if(iLegendY1 > iLegendY2)
		{
			iLegendY = iLegendY1;
		}
		else
...
@param sPlotDp         Input,  string,         the plot data point to read
@param ddsPlotData     Output, dyn_dyn_string, details of the plot are returned here (see fwTrending_PLOT_OBJECT.... constants for info on what is returned)
@param exceptionInfo   Output, dyn_string,     details of any exceptions are returned here
*/
fwTrending_getPlot(string sPlotDp, dyn_dyn_string &ddsPlotData, dyn_string &exceptionInfo)
{
  if( dpExists(sPlotDp) )
  {
    if( dpTypeName(sPlotDp) == fwTrending_PLOT )
    {
      dpGet(sPlotDp + fwTrending_PLOT_MODEL,          ddsPlotData[fwTrending_PLOT_OBJECT_MODEL][1],
            sPlotDp + fwTrending_PLOT_TITLE,          ddsPlotData[fwTrending_PLOT_OBJECT_TITLE][1],
            sPlotDp + fwTrending_PLOT_LEGEND_ON,      ddsPlotData[fwTrending_PLOT_OBJECT_LEGEND_ON][1],
            sPlotDp + fwTrending_PLOT_BACK_COLOR,     ddsPlotData[fwTrending_PLOT_OBJECT_BACK_COLOR][1],
            sPlotDp + fwTrending_PLOT_FORE_COLOR,     ddsPlotData[fwTrending_PLOT_OBJECT_FORE_COLOR][1],
            sPlotDp + fwTrending_PLOT_DPES,           ddsPlotData[fwTrending_PLOT_OBJECT_DPES],
            sPlotDp + fwTrending_PLOT_DPES_X,         ddsPlotData[fwTrending_PLOT_OBJECT_DPES_X],
            sPlotDp + fwTrending_PLOT_LEGENDS,        ddsPlotData[fwTrending_PLOT_OBJECT_LEGENDS],
            sPlotDp + fwTrending_PLOT_LEGENDS_X,      ddsPlotData[fwTrending_PLOT_OBJECT_LEGENDS_X],
            sPlotDp + fwTrending_PLOT_COLORS,         ddsPlotData[fwTrending_PLOT_OBJECT_COLORS],
            sPlotDp + fwTrending_PLOT_AXII,           ddsPlotData[fwTrending_PLOT_OBJECT_AXII],
            sPlotDp + fwTrending_PLOT_AXII_X,         ddsPlotData[fwTrending_PLOT_OBJECT_AXII_X],
            sPlotDp + fwTrending_PLOT_IS_TEMPLATE,    ddsPlotData[fwTrending_PLOT_OBJECT_IS_TEMPLATE][1],
            sPlotDp + fwTrending_PLOT_CURVES_HIDDEN,  ddsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN],
            sPlotDp + fwTrending_PLOT_RANGES_MIN,     ddsPlotData[fwTrending_PLOT_OBJECT_RANGES_MIN],
            sPlotDp + fwTrending_PLOT_RANGES_MAX,     ddsPlotData[fwTrending_PLOT_OBJECT_RANGES_MAX],
            sPlotDp + fwTrending_PLOT_RANGES_MIN_X,   ddsPlotData[fwTrending_PLOT_OBJECT_RANGES_MIN_X],
            sPlotDp + fwTrending_PLOT_RANGES_MAX_X,   ddsPlotData[fwTrending_PLOT_OBJECT_RANGES_MAX_X],
            sPlotDp + fwTrending_PLOT_TYPE,           ddsPlotData[fwTrending_PLOT_OBJECT_TYPE][1],
            sPlotDp + fwTrending_PLOT_TIME_RANGE,     ddsPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE][1],
            sPlotDp + fwTrending_PLOT_TEMPLATE_NAME,  ddsPlotData[fwTrending_PLOT_OBJECT_TEMPLATE_NAME][1],
            sPlotDp + fwTrending_PLOT_IS_LOGARITHMIC, ddsPlotData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC][1],
            sPlotDp + fwTrending_PLOT_GRID,           ddsPlotData[fwTrending_PLOT_OBJECT_GRID][1],
            sPlotDp + fwTrending_PLOT_CURVE_TYPES,    ddsPlotData[fwTrending_PLOT_OBJECT_CURVE_TYPES],
            sPlotDp + fwTrending_PLOT_MARKER_TYPE,    ddsPlotData[fwTrending_PLOT_OBJECT_MARKER_TYPE][1] );

      // add the "@" in case trendable information are aliases instead if datapoint names
      ddsPlotData[fwTrending_PLOT_OBJECT_DPES]   = _fwTrending_preparePlotObjectDPES(ddsPlotData[fwTrending_PLOT_OBJECT_DPES]);
      ddsPlotData[fwTrending_PLOT_OBJECT_DPES_X] = _fwTrending_preparePlotObjectDPES(ddsPlotData[fwTrending_PLOT_OBJECT_DPES_X]);

      if( dpExists(sPlotDp + fwTrending_PLOT_ACCESS_CONTROL_SAVE) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_ACCESS_CONTROL_SAVE, ddsPlotData[fwTrending_PLOT_OBJECT_ACCESS_CONTROL_SAVE][1]);
      }
      else
      {
        ddsPlotData[fwTrending_PLOT_OBJECT_ACCESS_CONTROL_SAVE][1] = "";
      }

      if( dpExists(sPlotDp + fwTrending_PLOT_CONTROL_BAR_ON) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_CONTROL_BAR_ON, ddsPlotData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON][1]);
      }
      else
      {
        ddsPlotData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON][1] = 0;
      }

      if( dpExists(sPlotDp + fwTrending_PLOT_AXII_POS) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_AXII_POS, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_POS]);
      }
      else
      {
        ddsPlotData[fwTrending_PLOT_OBJECT_AXII_POS] = makeDynString(2, 2, 2, 2, 2, 2, 2, 2); //2=SCALE_LEFT
      }

      if( dpExists(sPlotDp + fwTrending_PLOT_AXII_LINK) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_AXII_LINK, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK]);
        if( dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK]) == 0 )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK] = makeDynString(0, 0, 0, 0, 0, 0, 0, 0);
        }
      }
      else
      {
      ...
      if( dpExists(sPlotDp + fwTrending_PLOT_DEFAULT_FONT) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_DEFAULT_FONT, ddsPlotData[fwTrending_PLOT_OBJECT_DEFAULT_FONT][1]);
        if( ddsPlotData[fwTrending_PLOT_OBJECT_DEFAULT_FONT][1] == "" )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_DEFAULT_FONT][1] = fwTrending_DEFAULT_FONT;
        }
      }
      else
      {
      ...
      if( dpExists(sPlotDp + fwTrending_PLOT_CURVE_STYLE) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_CURVE_STYLE, ddsPlotData[fwTrending_PLOT_OBJECT_CURVE_STYLE][1]);
        if( ddsPlotData[fwTrending_PLOT_OBJECT_CURVE_STYLE][1] == "" )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_CURVE_STYLE][1] = fwTrending_DEFAULT_CURVE_STYLE;
        }
      }
      else
      {
      ...
      if( dpExists(sPlotDp + fwTrending_PLOT_AXII_X_FORMAT) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_AXII_X_FORMAT, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT]);
        if( dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT]) < 2 )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT] = makeDynString(fwTrending_DEFAULT_AXII_X_FORMAT, "", "", "", "", "", "", "");
        }
      }
      else
      {
        ddsPlotData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT] = makeDynString(fwTrending_DEFAULT_AXII_X_FORMAT, fwTrending_DEFAULT_AXII_X_FORMAT);
      }

      if( dpExists(sPlotDp + fwTrending_PLOT_LEGEND_VALUES_FORMAT) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_LEGEND_VALUES_FORMAT, ddsPlotData[fwTrending_PLOT_OBJECT_LEGEND_VALUES_FORMAT]);
        if( dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_LEGEND_VALUES_FORMAT]) < 8 )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_LEGEND_VALUES_FORMAT] = makeDynString("", "", "", "", "", "", "", "");
        }
      }
      else
      {
      ...
      if( dpExists(sPlotDp + fwTrending_PLOT_AXII_Y_FORMAT) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_AXII_Y_FORMAT, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_Y_FORMAT]);
        if( dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_AXII_Y_FORMAT]) < 8 )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_AXII_Y_FORMAT] = makeDynString("", "", "", "", "", "", "", "");
        }
      }
      else
      {
      ...
      if( dpExists(sPlotDp + fwTrending_PLOT_ALARM_LIMITS_SHOW) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_ALARM_LIMITS_SHOW, ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW]);
        if( dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW]) < 8 )
        {
          ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW] == makeDynString(0, 0, 0, 0, 0, 0, 0, 0);
        }
      }
      else
      {
      ...
      if( dpExists(sPlotDp + fwTrending_PLOT_SAVE_TO_USER_FOLDER) )
      {
        dpGet(sPlotDp + fwTrending_PLOT_SAVE_TO_USER_FOLDER, ddsPlotData[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER][1]);
      }
      else
      {
        ddsPlotData[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER] = makeDynBool(FALSE);
      }
    }
    else
    {
      fwException_raise(exceptionInfo, "ERROR", "fwTrending_getPlot(): " + sPlotDp + " is not of type " + fwTrending_PLOT, "");
    }
    
    if( ddsPlotData[fwTrending_PLOT_OBJECT_MODEL][1] == "" )
    {
      ddsPlotData[fwTrending_PLOT_OBJECT_MODEL][1] = fwTrending_YT_PLOT_MODEL;
    }
  }
  else
  {
    fwException_raise(exceptionInfo, "ERROR", "fwTrending_getPlot(): " + sPlotDp + " does not exist", "");
  }
...
@param sPlotDp        Input,  string,         the plot data point to write to
@param ddsPlotData    Input,  dyn_dyn_string, details of the plot are passed here (see fwTrending_PLOT_OBJECT.... constants for info on what is returned)
@param exceptionInfo  Output, dyn_string,     details of any exceptions are returned here
*/
fwTrending_setPlot(string sPlotDp, dyn_dyn_string ddsPlotData, dyn_string &exceptionInfo)
{
  dyn_errClass deSettingError;

  if( dpExists(sPlotDp) )
  {
    if( dpTypeName(sPlotDp) == fwTrending_PLOT )
    {
      ddsPlotData[fwTrending_PLOT_OBJECT_DPES]   = _fwTrending_cleanPlotObjectDPES(ddsPlotData[fwTrending_PLOT_OBJECT_DPES]);
      ddsPlotData[fwTrending_PLOT_OBJECT_DPES_X] = _fwTrending_cleanPlotObjectDPES(ddsPlotData[fwTrending_PLOT_OBJECT_DPES_X]);

      dpSetWait(sPlotDp + fwTrending_PLOT_MODEL,          ddsPlotData[fwTrending_PLOT_OBJECT_MODEL][1],
                sPlotDp + fwTrending_PLOT_TITLE,          ddsPlotData[fwTrending_PLOT_OBJECT_TITLE][1],
                sPlotDp + fwTrending_PLOT_LEGEND_ON,      ddsPlotData[fwTrending_PLOT_OBJECT_LEGEND_ON][1],
                sPlotDp + fwTrending_PLOT_BACK_COLOR,     ddsPlotData[fwTrending_PLOT_OBJECT_BACK_COLOR][1],
                sPlotDp + fwTrending_PLOT_FORE_COLOR,     ddsPlotData[fwTrending_PLOT_OBJECT_FORE_COLOR][1],
      ...
      if( dynlen(ddsPlotData) >= fwTrending_PLOT_OBJECT_CONTROL_BAR_ON )
      {
        if( dpExists(sPlotDp + fwTrending_PLOT_CONTROL_BAR_ON) && dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON]) > 0 )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_CONTROL_BAR_ON, (int)ddsPlotData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON][1]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_AXII_POS) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_AXII_POS, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_POS]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_AXII_LINK) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_AXII_LINK, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_DEFAULT_FONT) && dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_DEFAULT_FONT]) > 0 )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_DEFAULT_FONT, ddsPlotData[fwTrending_PLOT_OBJECT_DEFAULT_FONT][1]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_CURVE_STYLE) && dynlen(ddsPlotData[fwTrending_PLOT_OBJECT_CURVE_STYLE]) > 0 )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_CURVE_STYLE, ddsPlotData[fwTrending_PLOT_OBJECT_CURVE_STYLE][1]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_AXII_X_FORMAT) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_AXII_X_FORMAT, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_LEGEND_VALUES_FORMAT) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_LEGEND_VALUES_FORMAT, ddsPlotData[fwTrending_PLOT_OBJECT_LEGEND_VALUES_FORMAT]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_AXII_Y_FORMAT) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_AXII_Y_FORMAT, ddsPlotData[fwTrending_PLOT_OBJECT_AXII_Y_FORMAT]);
        }

        if( dpExists(sPlotDp + fwTrending_PLOT_ALARM_LIMITS_SHOW) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_ALARM_LIMITS_SHOW, ddsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW]);
        }

        deSettingError = getLastError();
        if( dynlen(deSettingError) > 0 )
        {
          throwError(deSettingError);
          fwException_raise(exceptionInfo, "ERROR", "fwTrending_setPlot(): Could not save plot data for " + sPlotDp, "");
        }
      }

      if( dynlen(ddsPlotData) >= fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER )
      {
        if( dpExists(sPlotDp + fwTrending_PLOT_SAVE_TO_USER_FOLDER) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_SAVE_TO_USER_FOLDER, ddsPlotData[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER][1]);
        }
      }


      if( dynlen(ddsPlotData) >= fwTrending_PLOT_OBJECT_ACCESS_CONTROL_SAVE )
      {
        if( dpExists(sPlotDp + fwTrending_PLOT_ACCESS_CONTROL_SAVE) )
        {
          dpSetWait(sPlotDp + fwTrending_PLOT_ACCESS_CONTROL_SAVE, ddsPlotData[fwTrending_PLOT_OBJECT_ACCESS_CONTROL_SAVE][1]);
        }
      }
      ...
      {
        throwError(settingError);
        fwException_raise(exceptionInfo, "ERROR", "fwTrending_setPlot(): Could not save plot access control settings for " + sPlotDp, "");
      }

    }
    else
    {
      fwException_raise(exceptionInfo, "ERROR", "fwTrending_setPlot(): " + sPlotDp + " is not of type " + fwTrending_PLOT, "");
    }
  }
  else
  {
    fwException_raise(exceptionInfo, "ERROR", "fwTrending_setPlot(): " + sPlotDp + " does not exist", "");
  }
...
fwTrending_getPlotExtended(string plotDp, string refName, dyn_dyn_string &plotExtendedData, dyn_string &exceptionInfo, bool returnCurveVisible = FALSE)
{
    ...
    plotExtendedData[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER][1] = oldStyleObject[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER];
...
fwTrending_getPlotData(string ref, string sDpName, dyn_string &returnValue)
{
    ...
    returnValue[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER] = plotData[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER][1];
  ...
  if( (sDpName != "") && (dpExists(sDpName + fwTrending_PLOT_LEGENDS)) )
  {  
  	dpGet(sDpName + fwTrending_PLOT_LEGENDS, dsLeg);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> fwTrending_getPlotData() -> sDpName "  + fwTrending_PLOT_LEGENDS + " DPE is empty or doesn't exists: " + sDpName + fwTrending_PLOT_LEGENDS);
    return;
  }
  ...
  if( (sDpName != "") && (dpExists(sDpName + fwTrending_PLOT_LEGENDS_X)) )
  {  
  	dpGet(sDpName + fwTrending_PLOT_LEGENDS_X, dsLeg);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> fwTrending_getPlotData() -> sDpName "  + fwTrending_PLOT_LEGENDS_X + " is empty or doesn't exists: " + sDpName + fwTrending_PLOT_LEGENDS_X);
    return;
  }
...
fwTrending_getTitle(string sDpName, string &sTitle, dyn_string &exceptionInfo)
{
  ...
  if( (sDpName != "") && (dpExists(sDpName + dpe)) )
  {
	  dpGet(sDpName + dpe, temp);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> fwTrending_getTitle() -> sDpName + dpe DPE is empty or doesn't exits: " + sDpName + dpe);
    return;
  }
...
fwTrending_trendRun(string ref, bool bRunTrend, string refControl)
{
	...
	{
		setValue(trendCaption + "trendCaption.currentTime", "visible", bRunTrend);
	}
							   ...
							   bool overwriteExisting, dyn_string &exceptionInfo, bool useDpNameForTitle = FALSE)
{
	...
	int i, length;
	dyn_dyn_string objectData, savedObjectData;
	...
	if(sDpType == fwTrending_PLOT)
	{
		...
		// keep the settings of the Controls Bar as saved from the Options panel (if it is set to hidden, then keep it as hidden)
// 		fwTrending_getPlot(sDpName, savedObjectData, exceptionInfo);
//   objectData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON] = savedObjectData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON];
...
fwTrending_pageExportTrend(dyn_string dsRef, string templateParameters, string sSave = "FALSE")
{
	...
	// select the file
	ChildPanelOnCentralReturn("fwTrending/fwTrendingExportFileSelector.pnl",
                              "Export data to CSV",
                              makeDynString("$sSaveToUserFolder:" + sSave), 
                              df,
                              saveOptions);
	...
	for(countRef = 1; countRef <= nbRef; countRef++)
	{
		...
		for(i = 1; i <= fwTrending_TRENDING_MAX_CURVE; i++)
		{
			...
			if(dsTemp[i] != "")
			{
				...
				if( (tempDpe != "") && (dpExists(tempDpe + ":_archive.._type")) )
				{
...
@param sFileName             input, file name to export the data to
...
@param sTemplateParameters   input, any template parameter substitutions
@param exceptionInfo         output, details of any exceptions are returned here
@param bFillEmptyCells       input, Optional parameter - default FALSE:
...
fwTrending_exportTrend(string sFileName, dyn_string dsCurveDPE, dyn_string dsLegendCurve,
                       dyn_time dtStart, dyn_time dtEnd, string sTemplateParameters,
                       dyn_string &exceptionInfo, bool bFillEmptyCells = FALSE)
{
  int iLoop, iLenOrdered, iLenDPE, iLoopInt, iLoopCount, iLenDPE, iBonus;
  file fFile;
  string sTextFile, sTempDPE, sDpConf;
  time tFirstTime, tLastTime;
  dyn_string dsDPE, dsLegend, dsFileDPE;
  dyn_time dtStartInt, dtEndInt, dtOrdered;
  dyn_float dfValue;
  dyn_int diEventType, diCurve;
  dyn_dyn_time ddtTime;
  dyn_dyn_float ddfValue;
  dyn_dyn_int ddiEventType;
  dyn_dyn_bool ddbValue;
  

  //SMS template mechanism
  _fwTrending_evaluateTemplate(sTemplateParameters, dsCurveDPE,    exceptionInfo);
  _fwTrending_evaluateTemplate(sTemplateParameters, dsLegendCurve, exceptionInfo);
  ...
  iLoop = 1;
  iLenDPE = dynlen(dsCurveDPE);
  for( iLoopCount = 1 ; iLoopCount <= iLenDPE ; iLoopCount++ )
  {
    if( dsCurveDPE[iLoopCount] != "" )
    {
      dsDPE[iLoop]      = dsCurveDPE[iLoopCount];
      dsLegend[iLoop]   = dsLegendCurve[iLoopCount];
      dtStartInt[iLoop] = dtStart[iLoopCount];
      dtEndInt[iLoop]   = dtEnd[iLoop];
      iLoop++;
    }
  }

  // Calculate time period
  tFirstTime = dynMin(dtStartInt);
  tLastTime  = dynMax(dtEndInt);
  iBonus     = bFillEmptyCells;


  // Check number of curve values
  iLenDPE    = dynlen(dsDPE);
  if( iLenDPE == 0 )
  {
    return;
  }
  ...
  fwTrending_rebuildDpeUNICOS(dsDPE, dsFileDPE);

  // Add the "@" in case trendable information are aliases instead of datapoint names
  dsDPE = _fwTrending_preparePlotObjectDPES(dsDPE);

  for( iLoop = 1 ; iLoop <= iLenDPE ; iLoop++ )
  {
    sDpConf = dpSubStr(dsDPE[iLoop], DPSUB_CONF);
    
    // In order to preserve dp as alias, see if we can avoid dpSubStr (it does not support dp as alias, it converts it to dp name!)
    if( sDpConf != "" && patternMatch("*" + sDpConf, dsDPE[iLoop]) )
    {
      sTempDPE = dpSubStr(dsDPE[iLoop], DPSUB_SYS_DP_EL);
    }
    else
    {
      sTempDPE = dsDPE[iLoop];
    }
    
    if( (sTempDPE != "") && (dpExists(sTempDPE)) )
    {
      dpGetPeriod(dtStartInt[iLoop], dtEndInt[iLoop], iBonus, sTempDPE + ":_offline.._value", dfValue, dtOrdered, diEventType);
      ...
      if( dynlen(dfValue) == 0 )
      {
        if( bFillEmptyCells )
        {
          dpGetAsynch(tFirstTime, sTempDPE + ":_offline.._value", ddfValue[iLoop][1]);
          ddtTime[iLoop][1] = tFirstTime - makeTime(0, 0, 0, 0, 0, 0, 1);
        }
        else
        {
          dpGet(sTempDPE + ":_offline.._value", ddfValue[iLoop][1],
                sTempDPE + ":_offline.._stime", ddtTime [iLoop][1]);
        }
      }
      else
      {
        ddtTime[iLoop]      = dtOrdered;
        ddfValue[iLoop]     = dfValue;
        ddiEventType[iLoop] = diEventType;
      }
      diCurve[iLoop] = 1;
    }
    else
    {
      ddfValue[iLoop][1] = 0.0;
      ddtTime[iLoop][1]  = 0;
      diCurve[iLoop]     = 1;
    }
  }
  ...
  // Add DPE file header
  sTextFile = "Year/Month/Day Hour:Minute:Second.Millisecond,";
  for( iLoop = 1 ; iLoop <= iLenDPE ; iLoop++ )
  {
    sTextFile += dsFileDPE[iLoop] + ",";
  }
  sTextFile += "\n";


  // Add LEGEND file header
  sTextFile += "Year/Month/Day Hour:Minute:Second.Millisecond,";
  for( iLoop = 1 ; iLoop <= iLenDPE ; iLoop++ )
  {
    sTextFile += fwTrending_modify_CSV_characters(dsLegend[iLoop]) + ",";
  }
  sTextFile += "\n";


  // Sort events by time
    // dtOrdered will contain list of sorted time stamps
    // ddbValue will contain lists of bools that represent which dsDPE had a value at that time
  fwTrending_eventsSort(ddtTime, ddbValue, dtOrdered);

  // Write data out to file in rows for each timestamp
  iLenOrdered = dynlen(dtOrdered);
  for( iLoopInt = 1 ; iLoopInt <= iLenOrdered ; iLoopInt++ )
  {
    // Write data, time and a comma
    string sTime, sDate;
    sTime  = formatTime("%H:%M:%S", dtOrdered[iLoopInt], ".%03d");
    sDate  = formatTime("%Y/%m/%d", dtOrdered[iLoopInt]);
    
    if( bFillEmptyCells && (iLoopInt != iLenOrdered) )
    {
      if( (dtOrdered[iLoopInt] < tFirstTime) || 
          (dtOrdered[iLoopInt] > tLastTime) )
      {
        continue;
      }
    }
    sTextFile += fwTrending_modify_CSV_characters(sDate) + " " + fwTrending_modify_CSV_characters(sTime) + ",";


    //check and write out values that have timestamp, dtOrdered[iLoopInt]
    for( iLoop = 1 ; iLoop <= iLenDPE ; iLoop++ )
    {
      if( ddbValue[iLoopInt][iLoop] )
      {
        //write out data and increment counter for next value to read for each dpe
        sTextFile += fwTrending_modify_CSV_characters(ddfValue[iLoop][diCurve[iLoop]++]) + ",";
      }
      else
      {
        if( bFillEmptyCells )
        {
          ...
          if( (diCurve[iLoop] - 1) == 0 )
          {
            sTextFile += fwTrending_modify_CSV_characters(ddfValue[iLoop][1]) + ",";
          }
          else
          {
            sTextFile += fwTrending_modify_CSV_characters(ddfValue[iLoop][diCurve[iLoop] - 1]) + ",";
          }
        }
        else
        {
  ...
  if(fFile == 0)
  {
    fwException_raise(exceptionInfo , "WARNING", "fwTrending: the file specified cannot be opened: " + sFileName, "");
    return;
  }
  
  fputs(sTextFile, fFile);
  fflush(fFile);
  fclose(fFile);
  
}// fwTrending_exportTrend()
...
@param sTextInput   input,  string to check and if any special character appears transform it.

@return string already transformed.
*/
string fwTrending_modify_CSV_characters(string sTextInput)
{
  bool bIsQuotes, bIsComma, bIsCRLF;
  string sTextInit, sTextEnd, sTextReplace, sTextOutput;

//   DebugTN("fwTrending_check_CSV_compatibility() -> sTextInput = " + sTextInput);
  ...
  sTextInit    = "";
  sTextEnd     = "";
  sTextReplace = sTextInput;

  // Find if input string has any special character to transform.
  if( strpos(sTextInput, "\"") >= 0 )
    bIsQuotes = TRUE;
  
  if( strpos(sTextInput, ",") >= 0 )
    bIsComma = TRUE;
  
  if( strpos(sTextInput, "\n") >= 0 )
    bIsCRLF = TRUE;
  
  // Add " " for the field
  if( bIsQuotes || bIsComma || bIsCRLF )
  {
    sTextInit = "\"";
    sTextEnd  = "\"";
    
    if( bIsQuotes )
      strreplace(sTextReplace, "\"", "\"\"");
  }
  
  sTextOutput = sTextInit + sTextReplace + sTextEnd;

//   DebugTN("fwTrending_check_CSV_compatibility() -> sTextOutput = " + sTextOutput);
  return(sTextOutput);
  
}// fwTrending_modify_CSV_characters()
		...
		dyn_string &exceptionInfo, bool runningInsideTrend = TRUE)
{
    ...
    convertedValues[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER] = plotExtendedObject[fwTrending_PLOT_OBJECT_SAVE_TO_USER_FOLDER][1];
...
_fwTrending_getConnectedSystemIds(dyn_int &systemIds, dyn_string &exceptionInfo)
{
  ...
  if( (sConnectionsDp != "") && (dpExists(sConnectionsDp + ".Dist.ManNums:_online.._value")) )
  {
	  dpGet(sConnectionsDp + ".Dist.ManNums:_online.._value", connectedDistMans);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> _fwTrending_getConnectedSystemIds() -> sConnectionsDp + .Dist.ManNums:_online.._value DPE is empty or doesn't exits: " + sConnectionsDp + ".Dist.ManNums:_online.._value");
    return;
  }
  ...
  if(dynlen(connectedDistMans) > 0)
	{
		dpGet(sConnectionsDp + ".Dist.ManNums:_online.._stime", time1,
			  sDistDp + ".State.SystemNums:_online.._stime", time2,
			  sDistDp + ".State.SystemNums:_online.._value", connectedSystemIds);
...
_fwTrending_updateHistogram(string dpa, dyn_float value)
{
  ...
  if( (dpe != "") && (dpExists(dpe + ":_online.._stime")) )
  {
  	dpGet(dpe + ":_online.._stime", update);
  }
  else
  {
    DebugTN("Error in fwTrending.ctl -> _fwTrending_updateHistogram() -> dpe + :_online.._stime DPE is empty or doesn't exists: " + dpe + ":_online.._stime");
    return;
  }
...
/**@brief Get the min and max values of all y-scales in the currently visible trend
...
@param [in] sReferenceName   The reference as string

@return dyn_string, min and max (format: 0:0) for all 8 possible curves

@see fwTrendingTrend.pnl
*/
dyn_string fwTrending_getCurrentScaleMinMaxRanges(string sReferenceName)
{
	...
	fwTrending_getRuntimePlotDataWithStrings(sReferenceName, sIsRunning, dsPlotShapes, dsPlotData, exceptionInfo);

	return dsPlotData[fwTrending_PLOT_OBJECT_EXT_MIN_MAX_RANGE];
}

/**@brief Get the min and max values of the x-scale in the currently visible value over value trend
...
@return dyn_string, min and max (format: 0:0) for x-scale in first array element
...
dyn_string fwTrending_getCurrentScaleMinMaxRangesX(string sReferenceName)
{
	...
	return dsPlotData[fwTrending_PLOT_OBJECT_EXT_MIN_MAX_RANGE_X];
}

/**@brief Get scale visibility all y-scales in the currently visible trend
...
@param [in] sReferenceName   string, the reference

@return dyn_string, "TRUE" or "FALSE" for all 8 possible curves
...
dyn_string fwTrending_getCurrentScaleVisibilities(string sReferenceName)
{
	...
	return dsPlotData[fwTrending_PLOT_OBJECT_AXII];
}

/**@brief Check if passed DPEs have no invalid values
...
@param [in] dsDpes   dyn_string, the DPEs to be checked

@return bool, TRUE if all passed values valid, FALSE if at least one value invalid
...
bool fwTrending_valuesAreValid(dyn_string dsDpes)
{
	bool bAreValid = TRUE;
	float fDpeValue;
	string sDpe, sShownLabel = "";
	dyn_string dsDpName;

	for(int i = 1; i <= dynlen(dsDpes); i++)
	{
		if(dpExists(dpSubStr(dsDpes[i], DPSUB_SYS_DP_EL)))
		{
			dpGet(dsDpes[i], fDpeValue);
			if(isnan(fDpeValue))
			{
				bAreValid = FALSE;

				if(dpGetAlias(dsDpes[i]) != "")
				{
					sShownLabel = dpGetAlias(dsDpes[i]);
				}
				else
				{
					dsDpName = strsplit(dpSubStr(dsDpes[i], DPSUB_SYS_DP_EL), ".");
					sDpe = dsDpName[dynlen(dsDpName)];
					sShownLabel = dpGetAlias(dpSubStr(dsDpes[i], DPSUB_SYS_DP) + ".") + "." + sDpe;
				}

				unGraphicalFrame_ChildPanelOnCentralModal("vision/MessageWarning", "Warning",
						makeDynString("$1:Current value for " + sShownLabel + " is not a number. This trend can only handle finite values, so it gave up."));
				break;
			}
		}
	}
...
/**@brief get the currently displayed settings and saves them
...
@see fwTrendingTrendControl.pnl
*/
...
/**@brief makes  snapshot of the plot and saves it into a file (selected by user)
...
@param activeTrendShape the trending shape
...
{
	dyn_string exceptionInfo;
	fwScreenShot_saveScreenShotToFile(exceptionInfo);
}


// ------------------------------------------------------------------------------------------------

/**@brief opens the zoomed trending window
...
@par Usage
	Public
@param ref the trending shape name
@param dsPlotData the trending data

@see fwTrendingTrendControl.pnl
*/
fwTrending_openZoomedWindow(string ref, dyn_string dsPlotData)
{
	string sCurrentParameterValues, sDpName;
	...
	// get the list of DPE
	dynAppend(dsParameters, "$dsCurveDPE:" + dsPlotData[fwTrending_PLOT_OBJECT_DPES]);
	dynAppend(dsParameters, "$dsCurveLegend:" + dsPlotData[fwTrending_PLOT_OBJECT_LEGENDS]
			  + dsPlotData[fwTrending_PLOT_OBJECT_LEGEND_VALUES_FORMAT]);
	// get the tooltiptext
	dynAppend(dsParameters, "$dsCurveToolTipText:" + dsPlotData[fwTrending_PLOT_OBJECT_EXT_TOOLTIPS]);

	// get the curves ranges, format min:max
	dynAppend(dsParameters, "$dsCurveRange:" + dsPlotData[fwTrending_PLOT_OBJECT_EXT_MIN_MAX_RANGE]);

	// get the colors and font
	dynAppend(dsParameters, "$dsCurveColor:" + dsPlotData[fwTrending_PLOT_OBJECT_COLORS]);
	dynAppend(dsParameters, "$sForeColor:" + dsPlotData[fwTrending_PLOT_OBJECT_FORE_COLOR]
			  + fwTrending_CONTENT_DIVIDER + dsPlotData[fwTrending_PLOT_OBJECT_DEFAULT_FONT]);
	dynAppend(dsParameters, "$sBackColor:" + dsPlotData[fwTrending_PLOT_OBJECT_BACK_COLOR]);

	// get the DPE unit
	dynAppend(dsParameters, "$dsUnit:" + dsPlotData[fwTrending_PLOT_OBJECT_EXT_UNITS]);

	// get the curve type
	dynAppend(dsParameters, "$dsCurvesType:" + dsPlotData[fwTrending_PLOT_OBJECT_CURVE_TYPES]);
	...
	dynAppend(dsParameters, "$dsCurveScaleVisible:"
			  + dsPlotData[fwTrending_PLOT_OBJECT_AXII]
			  + dsPlotData[fwTrending_PLOT_OBJECT_AXII_POS]
			  + dsPlotData[fwTrending_PLOT_OBJECT_AXII_LINK]
			  + dsPlotData[fwTrending_PLOT_OBJECT_AXII_Y_FORMAT]);

	// get the curve visibility
	dynAppend(dsParameters, "$dsCurveVisible:"
			  + dsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN]
			  + dsPlotData[fwTrending_PLOT_OBJECT_ALARM_LIMITS_SHOW]);

	dynAppend(dsParameters, "$bShowGrid:" + dsPlotData[fwTrending_PLOT_OBJECT_GRID]);
	dynAppend(dsParameters, "$iMarkerType:" + dsPlotData[fwTrending_PLOT_OBJECT_MARKER_TYPE]
			  + fwTrending_CONTENT_DIVIDER + dsPlotData[fwTrending_PLOT_OBJECT_CURVE_STYLE]);
	dynAppend(dsParameters, "$bShowLegend:" + dsPlotData[fwTrending_PLOT_OBJECT_LEGEND_ON]
			  + fwTrending_CONTENT_DIVIDER + dsPlotData[fwTrending_PLOT_OBJECT_CONTROL_BAR_ON]);

	// get minimum for log and maximum percentage for log
	dynAppend(dsParameters, "$fMinForLog:" + dsPlotData[fwTrending_PLOT_OBJECT_EXT_MIN_FOR_LOG]);
	dynAppend(dsParameters, "$fMaxPercentageForLog:" + dsPlotData[fwTrending_PLOT_OBJECT_EXT_MAX_PERCENTAGE_FOR_LOG]);
	...
	dynAppend(dsParameters, "$bTrendLog:" + dsPlotData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC]);

	// get the current time range (items 1-8), the time format (items 9-16)
	dynAppend(dsParameters, "$sTimeRange:" + dsPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE]
			  + fwTrending_CONTENT_DIVIDER + dsPlotData[fwTrending_PLOT_OBJECT_AXII_X_FORMAT]);

	// get the zoom window title
	strreplace(dsPlotData[fwTrending_PLOT_OBJECT_TITLE], "\\_", " ");
	if(dsPlotData[fwTrending_PLOT_OBJECT_TITLE] == "")
	{
		dsPlotData[fwTrending_PLOT_OBJECT_TITLE] = "Trend Zoom";
	}

	dynAppend(dsParameters, "$ZoomWindowTitle:" + dsPlotData[fwTrending_PLOT_OBJECT_TITLE]);

	getValue(ref + "trend.parameterValues", "text", sCurrentParameterValues);
	dynAppend(dsParameters, "$templateParameters:" + sCurrentParameterValues);

	// open the zoomed window
	ChildPanelOnCentral("fwTrending/fwTrendingZoomedWindow.pnl", "Zoomed Window: " + dsPlotData[fwTrending_PLOT_OBJECT_TITLE],
						dsParameters);
}

// ------------------------------------------------------------------------------------------------

/**@brief opens the trending configuration window
...
@see fwTrendingTrendControl.pnl
*/
fwTrending_openConfiguration(string ref, dyn_string dsPlotData)
{
	bool bIsLog, bEnabled;
	string trendRunning, sCurrentParameterValues, sDpName, sProposedName;
	dyn_float dfReturn;
	dyn_string dsReturn, dsParameters, dsPlotShapes, dsPanels, dsModifiedPlotData, dsOldCurves, dsVisibleCurves, exceptionInfo;
	dyn_dyn_string ddsExtendedPlotData;
	shape activeTrendShape, standardTrendShape;
	int iLength;

	getValue(ref + "trend.dpNameText", "text", sDpName);
	fwTrending_convertStringToDyn(dsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN], dsVisibleCurves, exceptionInfo);
	_fwTrending_switchCurvesHiddenVisible(dsVisibleCurves, exceptionInfo);
	fwTrending_convertDynToString(dsVisibleCurves, dsPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN], exceptionInfo);
	fwDevice_getDefaultConfigurationPanels(fwTrending_PLOT, dsPanels, exceptionInfo);

    // FWTREND-964
    // Get all curve scales, start time, end time from Trend Widget and pass to the panel as $ parameter.
    int iLoop;  
    time tStart, tEnd;
    dyn_float dfAxisMaxScale, dfAxisMinScale;

    for( iLoop = 1 ; iLoop <= fwTrending_TRENDING_MAX_CURVE ; iLoop++ )
    {
      getMultiValue(ref + "standardTrend", "curveMax", "curve_" + iLoop, dfAxisMaxScale[iLoop],
                    ref + "standardTrend", "curveMin", "curve_" + iLoop, dfAxisMinScale[iLoop]);
    }
    getValue(ref + "standardTrend", "visibleTimeRange", 0, tStart, tEnd);
    // END FWTREND-964

	ChildPanelOnModalReturn(dsPanels[1] + ".pnl", "Plot Configuration",
							makeDynString("$Command:onlineedit", "$sDpName:" + sDpName, "$dsPlotData:" + dsPlotData, "$dsShowButtons:" + makeDynString("ok", "cancel")),
							0, 0, dfReturn, dsReturn);

	if(dsReturn[1] != "Cancel")
	{
		strreplace(dsReturn[3], " | ", "|");
		dsModifiedPlotData = strsplit(dsReturn[3], "|");

		for(int plotIndex = 1; plotIndex <= fwTrending_SIZE_PLOT_OBJECT; plotIndex++)
		{
			dsModifiedPlotData[plotIndex] = strltrim(dsModifiedPlotData[plotIndex], " ");
			dsModifiedPlotData[plotIndex] = strrtrim(dsModifiedPlotData[plotIndex], " ");
		}

		fwTrending_convertStringToDyn(dsModifiedPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN], dsVisibleCurves, exceptionInfo);
		_fwTrending_switchCurvesHiddenVisible(dsVisibleCurves, exceptionInfo);
		fwTrending_convertDynToString(dsVisibleCurves, dsModifiedPlotData[fwTrending_PLOT_OBJECT_CURVES_HIDDEN], exceptionInfo);

		setValue(ref + "trend.trendInfo", "items", dsModifiedPlotData);

		fwTrending_getRuntimePlotDataWithExtendedObject(ref, trendRunning, dsPlotShapes, ddsExtendedPlotData, exceptionInfo, FALSE);
		activeTrendShape = getShape(dsPlotShapes[fwTrending_ACTIVE_TREND_NAME]);

		fwTrending_convertStringToDyn(dsPlotData[fwTrending_PLOT_OBJECT_DPES], dsOldCurves, exceptionInfo);
		iLength = dynlen(dsOldCurves);

		if(iLength > fwTrending_TRENDING_MAX_CURVE)
		{
			iLength = fwTrending_TRENDING_MAX_CURVE;
		}

		for(int curveIndex = 1; curveIndex <= iLength; curveIndex++)
		{
			if(dsOldCurves[curveIndex] != ddsExtendedPlotData[fwTrending_PLOT_OBJECT_DPES][curveIndex])
			{
				activeTrendShape.disconnectDirectly("curve_" + curveIndex);
				activeTrendShape.curveRemoveData("curve_" + curveIndex, getCurrentTime());
			}
		}

		if((ddsExtendedPlotData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC][1] == "TRUE")
				|| (ddsExtendedPlotData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC][1] == TRUE))
		{
			bIsLog = TRUE;
		}
		else
		...
		setValue(ref + "trend.activeTrendText", "text", dsPlotShapes[fwTrending_LINEAR_TREND_NAME]);
		dsPlotShapes[fwTrending_ACTIVE_TREND_NAME] = dsPlotShapes[fwTrending_LINEAR_TREND_NAME];

		getValue(ref + "trend.parameterValues", "text", sCurrentParameterValues);

		fwTrending_checkAndGetAllTemplateParameters(ref, ddsExtendedPlotData, fwTrending_PLOT, sCurrentParameterValues, exceptionInfo, TRUE);
		setValue(ref + "trend.parameterValues", "text", sCurrentParameterValues);

		_fwTrending_preparePlotObjectForDisplay(ddsExtendedPlotData, sCurrentParameterValues, exceptionInfo);

		getValue("saveSettings", "enabled", bEnabled);
		fwTrending_initTrendWithObject(ref, dsPlotShapes[fwTrending_ACTIVE_TREND_NAME], dsPlotShapes, ddsExtendedPlotData, trendRunning, TRUE, exceptionInfo, TRUE);
		fwTrending_showStandardTrend(!bIsLog, dsPlotShapes[fwTrending_LINEAR_TREND_NAME], dsPlotShapes[fwTrending_LOG_TREND_NAME], dsPlotData);
		fwTrending_trendUpdateScales(ref, ddsExtendedPlotData, dsPlotShapes[fwTrending_ACTIVE_TREND_NAME], exceptionInfo);
		fwTrending_initControlTrendButtons(ddsExtendedPlotData[fwTrending_PLOT_OBJECT_TIME_RANGE][1], ref, sDpName, ddsExtendedPlotData[fwTrending_PLOT_OBJECT_IS_LOGARITHMIC][1], false);

		// connect to alarm handling for those dps showing the alarm limits on the trend
		_fwTrending_connectToAlarmHandling(ref, ddsExtendedPlotData);
		setValue("saveSettings", "enabled", bEnabled);
	}

  // FWTREND-964
  // Recover scale and timewindow
  for( iLoop = 1 ; iLoop <= fwTrending_TRENDING_MAX_CURVE ; iLoop++ )
  {
    setMultiValue(ref + "standardTrend", "curveMax", "curve_" + iLoop, dfAxisMaxScale[iLoop],
                  ref + "standardTrend", "curveMin", "curve_" + iLoop, dfAxisMinScale[iLoop]);
  }
  setValue(ref + "standardTrend", "visibleTimeRange", 0, tStart, tEnd);
  // END FWTREND-964
...
/**@brief opens the trending "Save As" dialog window
...
{
	string sDpName, sProposedName;

	setValue(ref + "trend.refreshTrend", "text", "FALSE");
	getValue(ref + "trend.dpNameText", "text", sDpName);
//    DebugN("fwTrending_save","sDpName:",sDpName);
	if(!patternMatch("_*", dpSubStr(sDpName, DPSUB_DP))) //it is not an internal dp (i.e. starting with "_") --> Save
	{
		fwTrending_getAndSaveFromTrendWidget();
		fwTrending_pageSavePlotSettings(ref);
	}
	else //it is an internal dp (i.e. starting with "_") --> Save As
	{
		string sTrendRunning;
		dyn_string dsPlotShapes, dsPlotData, exceptionInfo;
		fwTrending_getRuntimePlotDataWithStrings(ref, sTrendRunning, dsPlotShapes, dsPlotData, exceptionInfo, FALSE);
		fwTrending_saveAs(ref, dsPlotData);
	}
	delay(0, fwTrending_REFRESH_DELAY);
...
@see fwTrendingTrendControl.pnl
*/
fwTrending_saveAs(string ref, dyn_string dsPlotData)
{
	string sDpName, sProposedName;
	dyn_float dfReturn;
	...
	setValue(ref + "trend.refreshTrend", "text", "FALSE");
	getValue(ref + "trend.dpNameText", "text", sDpName);
	sProposedName = dpGetAlias(sDpName + ".");

	if(sProposedName == "")
	{
		sProposedName = dpSubStr(sDpName, DPSUB_DP);
	}

	if(sProposedName == "")
	{
		sProposedName = dsPlotData[fwTrending_PLOT_OBJECT_TITLE];
	}

	ChildPanelOnCentralReturn("fwTrending/fwTrendingSaveAsSelector.pnl", "Save plot settings as...",
							  makeDynString("$sDpType:" + fwTrending_PLOT, "$sProposedName:" + sProposedName),
							  dfReturn,
							  dsReturn);

	if(dsReturn[1] == "ok")
	{
		fwTrending_saveCurrentSettings(ref, dsReturn[2], fwTrending_PLOT, TRUE, TRUE, exceptionInfo, TRUE);
	}
...
/**@brief enable disable logarithmic scale
...
@par Usage
	Public
@param state true for logarithmic, false for linear
...
fwTrending_toggleLog(bool state)
{
	...
	if(state)
	{
		...
		delay(0, 300);
		fwTrendingControl_autoScalePushed(true);
	}
	else
	{
		fwTrendingControl_autoScalePushed(true);
...
/**@brief event callback for enable disable auto scale
...
@par Usage
	Public
@param state true for autoscale
...
fwTrendingControl_autoScalePushed(bool state)
{
	...
	fwTrendingTrendControl_setAutoscale(plotShapes[fwTrending_ACTIVE_TREND_NAME], state,
...
@see fwTrendingTrendControl.pnl
*/
fwTrendingTrendControl_setAutoscale(string trendName, bool state, dyn_string dsCurveRange,
...
/**@brief enable disable Y scale for a selected curve
...
@par Usage
	Public
@param ref  trending shape name
@param axii axii status (visible or not)
@param curveNumber number of the curve whose visibility must be toggled
@param dsDpe dpes of the curves
@param dsLegend legends
...
@par Usage
	Public
@param t  encoded time in seconds
@param d days
@param h hours
@param m minutes
...
/**@brief decode time from seconds to d,h,m
